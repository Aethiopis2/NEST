/**
 * @brief The NES CPU core is based on the 6502 processor and runs at 
 *	approximately 1.79 MHz (1.66 MHz in a PAL NES). It was an 8-bit processor 
 *	that was degined by Ricoh and lacks MOS6502 Decimal flag. It had 12 addressing
 *	modes with 56 documented (legal opcodes). 
 *
 *  There are only limited number of register sets and sizes:
 *		A : accumulator register (8-bit)
 *		X : Index register (8-bit)
 *		Y : Index Y register (8-bit)
 *		sp: stack pointer (8-bit)
 *		pc: program counter/Instruction pointer (16-bit)
 *
 *  The CPU does the following steps in a loop (at the tick of every
...
private:
	...
	// I'll capture all "unofficial" opcodes with this function. It is
